...
const generateGoodsReport = async (groupedData, config) => {
    ...
    worksheet.getCell('E1').value =
        `(Từ ${config.dateRange.startDate} đến ${config.dateRange.endDate})`
    ...
    for (const dateKey of sortedDates) {
        ...
        const sortedReceiptKeys = Array.from(dayData.receipts.keys()).sort(
            (a, b) => a - b,
        )
        ...
        for (const receiptKey of sortedReceiptKeys) {
            ...
            receiptData.lineItems.forEach((item, index) => {
                const displayStatus =
                    config.statusMap[item.status] || item.status
            ...
            if (numItems > 1) {
                config.mergeableColumnKeys.forEach((key) => {
                    const colLetter = String.fromCharCode(
                        65 + config.columnKeys.indexOf(key),
                    )
                    worksheet.mergeCells(
                        `${colLetter}${startRow}:${colLetter}${startRow + numItems - 1}`,
                    )
                })
            }
        ...
        const dailyTotalRow = worksheet.addRow([
            null,
            null,
            null,
            null,
        ...
        worksheet.mergeCells(
            `A${dailyTotalRow.number}:I${dailyTotalRow.number}`,
        )
