...
const deleteTemporaryGoods = async () => {
  ...
  try {
    ...
    await GoodsAdvanceDetailModel.deleteMany({ goodsAdvanceId: { $in: tempAdvanceIds } });
    await GoodsAdvanceProcessModel.deleteMany({ goodsAdvanceId: { $in: tempAdvanceIds } });
    ...
    await GoodsReceiptDetailModel.deleteMany({ goodsReceiptId: { $in: tempReceiptIds } });
    await GoodsReceiptApprovalModel.deleteMany({ goodsReceiptId: { $in: tempReceiptIds } });
    ...
    await GoodsIssueDetailModel.deleteMany({ goodsIssueId: { $in: tempIssueIds } });
    await GoodsIssueApprovalModel.deleteMany({ goodsIssueId: { $in: tempIssueIds } });
